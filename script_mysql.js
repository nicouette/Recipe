...
function getRecipie(id, callback) {
  ...
  exeQuery('select nom, description from recettes where id =:id', { id }, function (resSelectedRecette) {
    ...
    exeQuery('select nom,quantité,unité from ingredients join recettes_ingredients on id=ingredient_id where recette_id=:id', { id }, function (resSelectedIngredient) {
...
function addRecipie(nom, description, type, callback) {
  ...
  exeQuery('insert into Recettes (nom,description,type) Values(:nom,:description,:type)', { nom, description, type }, function (results) {
